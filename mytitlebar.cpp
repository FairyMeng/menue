...
#define BUTTON_HEIGHT 28		// 按钮高度;
#define BUTTON_WIDTH 28			// 按钮宽度;
...
// 窗口是否支持拉伸;
// 在MyTitleBar类里面再次提供此接口是因为在baseWindow中MyTitleBar会遮挡窗口顶部;
// 导致鼠标在MyTitleBar上移动时，捕捉不到mouseMoveEvent事件;
// 也就获取不到当前鼠标的坐标，判断不了鼠标是否在可拉伸区域;
void MyTitleBar::setSupportStretch(bool isSupportStretch)
{
    // 对MyTitleBar的每个控件进行setMouseTracking;
